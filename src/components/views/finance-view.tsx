...
function FeeEditDialog({
  ...
  const [form, setForm] = useState({
    ...
    effectiveTo: fee?.effectiveTo ? new Date(fee.effectiveTo).toISOString().slice(0, 10) : '',
  })
  const [saving, setSaving] = useState(false)
  ...
  const handleSave = async () => {
    ...
    try {
      const payload = {
        ...form,
        feeMax: form.feeMax || null,
        effectiveFrom: form.effectiveFrom || undefined,
        effectiveTo: form.effectiveTo || undefined,
      }
      if (fee) {
        // PATCH — обновить существующую комиссию
        await apiPatch(`/api/finance/banks/${bankId}/fees`, { feeId: fee.id, ...payload })
  ...
  const handleDelete = async () => {
    ...
    setSaving(true)
    try {
      const res = await fetch(`/api/finance/banks/${bankId}/fees?feeId=${fee.id}`, { method: 'DELETE' })
      if (res.ok) {
        toast.success('Комиссия архивирована')
        onSaved()
        onClose()
      } else {
      ...
      <DialogContent className="max-w-md">
        ...
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div className="space-y-1.5">
              <Label className="text-xs">Действует по (пусто = бессрочно)</Label>
              <Input
                type="date"
